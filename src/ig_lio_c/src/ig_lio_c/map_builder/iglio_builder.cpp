#include "../../../include/ig_lio_c/map_builder/iglio_builder.h"
...
namespace IG_LIO
{
    ...
    void IGLIOBuilder::fastlioConstraint(IG_LIO::State &state, IG_LIO::SharedState &shared_state)
    {
    ...
    void IGLIOBuilder::gicpConstraint(IG_LIO::State &state, IG_LIO::SharedState &shared_state)
    {
        ...
        for (int i = 0; i < gicp_cache_.size(); i++)
        {
            ...
            Eigen::Matrix3d robust_information_matrix = params_.gicp_constraint_gain * (rho[1] * omiga + 2.0 * rho[2] * omiga * error * error.transpose() * omiga);
            shared_state.H += (J.transpose() * robust_information_matrix * J);
            shared_state.b += (params_.gicp_constraint_gain * rho[1] * J.transpose() * omiga * error);
        }
